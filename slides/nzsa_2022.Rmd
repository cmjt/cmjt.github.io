...
subtitle: "Charlotte M. Jones-Todd &nbsp;&nbsp; ![](img/twitter.svg){width=3%} <span style='color:#FFBF00'>@cmjonestodd</span>"
...
$$\mu = \int_{{\rm I\!R^{k+1}}} \Lambda(x)dx$$ 
where $$\Lambda(x) = \textrm{exp}(\bf{X}\beta + Z(x))$$ 
...
<br>
 ...
 + The $\alpha_{m_j}$ are coefficient(s) linking the point process and the $\text{n}_\text{mark}$ mark(s), $m_j$ $(j = 1, ..., \text{n}_\text{mark})$

 + The form of $M_j(s)$ and $f^{-1}()$ depends on the assumed distribution of the marks
...
<br>
 ...
 + Expected number of *triggered sightings* per sighting $\frac{\hat{\beta}}{\hat{\beta} - \hat{\alpha}} = \frac{0.09}{0.09 - 0.06} = 3$
...
.pull-right[
.center[`r fontawesome::fa("circle-question")` 

[<img src="http://www.bfro.net/images/templates/LogoSpin.gif?a;a;a;" width ="20%" />](http://www.bfro.net/)]
<br>
